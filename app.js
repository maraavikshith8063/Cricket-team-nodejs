...
module.exports = app.get("/players/", async (request, response) => {
  const getCricketQuery = `
    ...
    `;
  const cricketPlayers = await db.all(getCricketQuery);
  response.send(cricketPlayers);
});
...
module.exports = app.get("/players/:playerId/", async (request, response) => {
  ...
  const getSinglePlayer = `
    ...
    `;
  const singlePlayer = await db.get(getSinglePlayer);
...
module.exports = app.delete("/players/:playerId", async (request, response) => {
  const { playerId } = request.params;
